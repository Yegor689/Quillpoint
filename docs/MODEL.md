...
| `ReportView` | Day-by-day log of tasks completed in a chosen date range (presets + custom), with a created/completed/active-days summary. Opened from a toolbar button. Grouping is a pure `ReportBuilder` (in `ReportData.swift`), unit-tested. Only root tasks are rows; completed subtasks nest under the parent they finished with, behind an inline chevron that starts collapsed (so a collapsed row is no taller than a row without subtasks). Because a parent completes exactly when its last subtask does, subtasks finished on earlier days aren't shown on those days — they appear with the parent on its completion day |
...
| `AppSettings` | Persisted user preferences (theme, accent, task defaults, project-delete confirmation, reminder snooze/preset/hour/sound, sidebar and on-launch options), surfaced in the tabbed Settings window |
